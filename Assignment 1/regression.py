...
#%% Step 2
def feature_selection_linear(data, y, training_test_split, training_val_split):
    ...
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=training_test_split, shuffle=False)
    X_train, X_val, y_train, y_val = train_test_split(X_train, y_train, test_size=training_val_split, shuffle=False)
...
def cvL2(X_train, y_train, lb=0, ub=100):
    ...
    for l in lambda_:
        ...
        for k in range(K):
            xx_train, xx_val, yy_train, yy_val = train_test_split(xx_train, yy_train, test_size=1 / ((K + 1) - k),
                                                                  shuffle=False)
