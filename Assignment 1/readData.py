...
temp_dir = os.path.join(cwd, 'ClimateData/mean_wind_speed.json')
with open(temp_dir) as data_file:
    wind = json.load(data_file)

temp_dir = os.path.join(cwd, 'ClimateData/mean_temp.json')
with open(temp_dir) as data_file:
    temp = json.load(data_file)

""" Create dataframe with datetime observations """
time, time_check, wind_speed, temperature = [], [], [], []
...
df = pd.DataFrame([time,wind_speed,temperature]).T
df.columns = 'HourUTC','wind_speed [m/s]','temperature [C]'
